...
현재 버전은 **v0.4.0-beta — 관리자 대시보드 · 플레이 분석 (beta 미러)**입니다.
브랜드 표기는 `eslo`(영문) / `이슬로`(한글)로 통일합니다.

> ⚙️ 이 저장소는 운영본(`eslobaby-game2`)과 분리된 **beta 미러**(원격: `eslobaby-game-beta`)입니다.
> v0.4.0-beta: 운영자용 **관리자 대시보드** 추가 — 우측 하단 톱니바퀴 → 비밀번호 로그인 →
> 플레이 통계/STEP 퍼널/체류시간/기기 분포/오류 로그. 저장은 LocalStorage(추후 Firebase 확장).
> 비밀번호·표시 여부는 `config.admin`에서 관리. 일반 사용자에게는 노출되지 않음.
> (v0.3.3-beta: 거품+계면이 동시 생성·STEP1 샤워 잔류·STEP3 모두 제거·`Page N/10` 표시)
